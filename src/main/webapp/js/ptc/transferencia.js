...
function ir_reversion_transferencias_ptc() {
    window.location.hash = "IRRTPTC";
    $.ajax({
        type: "POST",
        url: ruta_contenedores_ptc + "contenedor_registro_reversion_trasnferencias.jsp",
...
function consultar_transferencias_reversiones_ptc() {
    $.ajax({
        type: "POST",
        url: rutaApiConsultaMaeharaPtc + "consultaTransferenciasCancelaciones.jsp",
        ...
        {
            $("#divContenedor").html("");
            $("#divContenedor").html(data.grilla);
            $("#grilla_transferencias").DataTable()
            cerrar_load();
...
function cancelar_transferencias_ptc(id, origen, destino,origenRes) {
    Swal.fire({

        title: 'Cancelacion de pedido nro. ' + id,
        type: 'warning',
        html: '<table class="table"><thead><tr><th>Origen</th><th>Destino</th></tr><tr><th>' + origen + '</th><th>' + destino + '</th></tr></thead></table>',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Aceptar cancelacion',
        cancelButtonText: 'No'}).then((result) =>
    {
        if (result.value)
        {
            $.ajax({
             type: "POST",
             url: rutaApiCrudMaeharaPtc + "crudCrearCancelacionTransferencia.jsp",
             data: ({id: id, origen: origenRes }),
             beforeSend: function ()
             {
             Swal.fire({
             title: 'PROCESANDO!',
             ...
             success: function (res)
             {
                ...
                if(res.tipo_respuesta==1){
                    consultar_transferencias_reversiones_ptc();
                }
             } ,
             error: function(XMLHttpRequest, textStatus, errorThrown) {
             if(XMLHttpRequest.status==404 || XMLHttpRequest.status==500){
...
function reprocesar_transferencias_ptc(id, origen, destino) {
    $.ajax({
        type: "POST",
        url: rutaApiConsultaMaeharaPtc + "consultaSelectAreas.jsp",
        data: ({origen: origen, destino: destino}),
        success: function (res)
        {
           const option=res.lista;
           
        Swal.fire({

        title: 'Reprocesar transferencia nro. ' + id,
        type: 'warning',
        html: '<table class="table"><thead><tr><th>Origen</th><th>Destino</th></tr><tr><th>' + origen + '</th><th>' + destino + '</th></tr></thead></table> \n\
            <br><a>Seleccionar nuevo destino</a> <br><select id="selectAreas" ><option value="">Selecciona</option>'+option+' </select>',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        cancelButtonText: 'No'}).then((result) =>
    {
        if (result.value)
        {
            var nuevoDestino=$("#selectAreas").val()

            if(nuevoDestino==""){
                aviso_generico(0, "Seleccione nuevo destino");
            }
            else{
               
            $.ajax({
             type: "POST",
             url: rutaApiCrudMaeharaPtc + "crudCrearReprocesoCancelacionTransferencia.jsp",
             data: ({id: id, nuevoDestino: nuevoDestino }),
             beforeSend: function ()
             {
             Swal.fire({
             title: 'PROCESANDO!',
...
function visualizar_lotes_cancelacion_transferencias_ptc(id) {

    $.ajax({
        type: "POST",
            url: rutaApiConsultaMaeharaPtc + "consultaTransferenciasCancelacionesLotes.jsp",
        data: ({id: id}),
        success: function (res)
        {
           const tabla=res.grilla;
           
            Swal.fire({

            title: 'Transferencia nro. ' + id,
            type: 'warning',
            html: '<div id="div_grilla_lotes" >'+tabla+'</div>',
             confirmButtonColor: '#3085d6',
             confirmButtonText: 'Cerrar'  }) 
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
