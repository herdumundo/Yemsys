...
function consultar_detalle_directorio_pedidos()
{
    $.ajax({
        type: "POST",
        url: ruta_consultas + "generar_grilla_preembarque_directorio_detalle.jsp",
        beforeSend: function ()
        {
...
cancelButtonText: 'Cerrar' 
            ...
            $("#tbDetalle").DataTable({paging: false, searching: false, info: false});
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            if (XMLHttpRequest.status == 404 || XMLHttpRequest.status == 500) {
                recargar_pagina();
